...
var Admin = class {
  ...
  async publish(recipient, payload) {
    const client = this.client;
    const format = client.options.useBinaryProtocol ? "msgpack" /* msgpack */ : "json" /* json */, headers = defaults_default.defaultPostHeaders(client.options, {
      format
    }), params = {};
    const body = mixin({
      recipient
...
var DeviceRegistrations = class {
  ...
  async remove(deviceIdOrDetails) {
    const client = this.client, format = client.options.useBinaryProtocol ? "msgpack" /* msgpack */ : "json" /* json */, headers = defaults_default.defaultGetHeaders(client.options, {
      format
    }), params = {}, deviceId = deviceIdOrDetails.id || deviceIdOrDetails;
    if (typeof deviceId !== "string" || !deviceId.length) {
...
var RestPresence = class {
  ...
  async get(params) {
    ...
    return new paginatedresource_default(client, this.channel.client.rest.presenceMixin.basePath(this), headers, envelope, async (body, headers2, unpacked) => {
      return await fromResponseBody2(body, options, this.logger, client._MsgPack, unpacked ? void 0 : format);
    }).get(params);
  }
...
var RestChannelMixin = class {
  ...
  static history(channel, params) {
    ...
    return new paginatedresource_default(client, this.basePath(channel) + "/messages", headers, envelope, async function(body, headers2, unpacked) {
      return await fromResponseBody(body, options, channel.logger, client._MsgPack, unpacked ? void 0 : format);
    }).get(params);
  }
...
var RestPresenceMixin = class {
  ...
  static async history(presence, params) {
    ...
    return new paginatedresource_default(client, this.basePath(presence) + "/history", headers, envelope, async function(body, headers2, unpacked) {
      return await fromResponseBody2(body, options, presence.logger, client._MsgPack, unpacked ? void 0 : format);
    }).get(params);
  }
};
...
var Protocol = class extends eventemitter_default {
  ...
  send(pendingMessage) {
    ...
    if (this.logger.shouldLog(logger_default.LOG_MICRO)) {
      logger_default.logActionNoStrip(this.logger, logger_default.LOG_MICRO, "Protocol.send()", "sending msg; " + stringify(pendingMessage.message, this.transport.connectionManager.realtime._RealtimePresence));
    }
...
var Transport = class extends eventemitter_default {
  ...
  onProtocolMessage(message) {
    if (this.logger.shouldLog(logger_default.LOG_MICRO)) {
      logger_default.logActionNoStrip(this.logger, logger_default.LOG_MICRO, "Transport.onProtocolMessage()", "received on " + this.shortName + ": " + stringify(message, this.connectionManager.realtime._RealtimePresence) + "; connectionId = " + this.connectionManager.connectionId);
    }
    this.onActivity();
...
var RealtimeChannel = class _RealtimeChannel extends eventemitter_default {
  ...
  async processMessage(message) {
    ...
    switch (message.action) {
      ...
      case actions2.PRESENCE: {
        ...
        for (let i = 0; i < presence.length; i++) {
          try {
            ...
            logger_default.logAction(this.logger, logger_default.LOG_ERROR, "RealtimeChannel.processMessage()", e.toString());
          }
        }
        if (this._presence) {
      ...
      default:
        logger_default.logAction(this.logger, logger_default.LOG_ERROR, "RealtimeChannel.processMessage()", "Fatal protocol error: unrecognised action (" + message.action + ")");
    ...
    try {
      ...
      if (items && items.length)
        for (let i = 0; i < items.length; i++)
          this.onProtocolMessage(fromDeserialized(items[i], this.connectionManager.realtime._RealtimePresence));
    } catch (e) {
...
var decodePresenceMessage = (obj, options) => {
  return fromEncoded2(logger_default.defaultLogger, obj, options);
};
var decodePresenceMessages = (obj, options) => {
  return fromEncodedArray2(logger_default.defaultLogger, obj, options);
};
