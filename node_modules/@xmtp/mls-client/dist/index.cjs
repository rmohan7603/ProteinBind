...
var mlsClientBindingsNode = require('@xmtp/mls-client-bindings-node');
...
class Client {
    ...
    static async create(accountAddress, options) {
        ...
        const inboxId = (await mlsClientBindingsNode.getInboxIdForAddress(host, isSecure, accountAddress)) ||
            mlsClientBindingsNode.generateInboxId(accountAddress);
        return new Client(await mlsClientBindingsNode.createClient(host, isSecure, dbPath, inboxId, accountAddress, options?.encryptionKey), [new GroupUpdatedCodec(), new contentTypeText.TextCodec(), ...(options?.codecs ?? [])]);
    }
    get accountAddress() {
    ...
    addEcdsaSignature(signatureBytes) {
        this.#innerClient.addEcdsaSignature(signatureBytes);
    }
    addScwSignature(signatureBytes, chainId, accountAddress, chainRpcUrl, blockNumber) {
        this.#innerClient.addScwSignature(signatureBytes, chainId, accountAddress, chainRpcUrl, blockNumber);
    }
