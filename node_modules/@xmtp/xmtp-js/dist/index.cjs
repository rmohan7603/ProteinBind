...
class ConsentList {
    ...
    async publish(entries) {
        ...
        // encoded action
        const payload = proto.privatePreferences.PrivatePreferencesAction.encode(action).finish();
        // encrypt payload
...
class Contacts {
    ...
    async allowGroups(groupIds) {
        await this.consentList.publish(groupIds.map((groupId) => ConsentListEntry.fromGroupId(groupId, 'allowed')));
    }
    async denyGroups(groupIds) {
        await this.consentList.publish(groupIds.map((groupId) => ConsentListEntry.fromGroupId(groupId, 'denied')));
    }
    async allowInboxes(inboxIds) {
        await this.consentList.publish(inboxIds.map((inboxId) => ConsentListEntry.fromInboxId(inboxId, 'allowed')));
    }
    async denyInboxes(inboxIds) {
        await this.consentList.publish(inboxIds.map((inboxId) => ConsentListEntry.fromInboxId(inboxId, 'denied')));
    }
}
